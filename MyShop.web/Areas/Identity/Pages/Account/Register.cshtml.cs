    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // to take this data extended from  application user 
                ...
                if (result.Succeeded)
                {
                    ...
                    // Check if there data name end with url with RoleRadio 
                    var role = HttpContext.Request.Form["RoleRadio"].ToString();

                    if(String.IsNullOrEmpty(role))
                    {
                        // add role customer for any new user registed
                        // ازل مره بس هعملها ادمن بعد كده زبون عادي عشام اعرف اظبط الرولز
                        await _userManager.AddToRoleAsync(user, Helpers.CustomerRole);

                        await _signInManager.SignInAsync(user, isPersistent: false);
                        return LocalRedirect(returnUrl);
                    }
                    else // mean the user is admin and will asign the role in Radio
                    {
